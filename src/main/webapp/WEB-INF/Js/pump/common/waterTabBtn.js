			...
			refreshPage : function(div) {
				...
				$.each(fn.config.arr,function(i,n){
					if (i >= ((fn.config.page-1)*fn.config.rowCount) && i < ((fn.config.page)*fn.config.rowCount)) {
						...
						var li = $("<li target='"+i+"' style='height:"+(liheight-4)+"px;line-height:"+(liheight-4)+"px;margin-top:2px;"+("" != fn.config.singleWidth ? "width:"+fn.config.singleWidth +";": "")+"' ><a style='height:"+(liheight-4)+"px;'>"+n[fn.config.textField]+"</a></li>");
